...
## MPV
```
mpv --image-display-duration=5 --osd-level=0 --no-input-cursor --no-input-default-bindings --loop-playlist=inf --playlist=./playlist.m3u
```

## VLC
```
vlc --image-duration=5 --no-video-title-show --no-qt-fs-controller --loop ./playlist.m3u
```
