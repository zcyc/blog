...
| Qt                   | C++ / QML / Python 等       | 是             | 是         | 是         | 原生（高性能，RAM 低 ~50MB）      | 成熟（Qt 公司，企业生态）       | 活跃  | 优点：高效、嵌入式支持；缺点：C++ 学习曲线。适合高性能 / 工业 APP（如 Autodesk）。 |
...
- **非主流框架**：NW.js（Electron 前身），SWT（用于 Eclipse），Dear ImGui（用于调试器和编辑器），egui、Iced、gpui（Rust 生态），Wails、Fyne（Go 生态）。
